...
function AnimationContainer() {
  return (<div className="w-full h-full relative font-sans text-slate-800 pointer-events-none">
    {/* SVG Layer */}
    ...
    <div className="absolute inset-0 z-20 pointer-events-none">
      ...
      {/* Source: Robot */}
      <div className="absolute top-1/2 -translate-y-1/2 flex items-center justify-center flex-col gap-3 pointer-events-auto" style={{ left: COL_1_X - 40 }}>
      ...
      {leftNodes.map((node, i) => {
        ...
        const startY = CENTER_Y - ((leftNodes.length - 1) * spacing) / 2;
        const y = startY + i * spacing;
        return (<motion.div key={`ln-${i}`} className="absolute flex items-center gap-3 p-2 pr-4 bg-white rounded-lg border border-gray-100 shadow-sm z-20 w-[170px] pointer-events-auto" style={{ left: COL_2_X, top: y - 20 }} initial={{ x: -20, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.5 + (i * 0.05), duration: 0.4 }}>
      ...
      {/* Center Dashboard */}
      {/* Center Dashboard */}
      <motion.div className="absolute bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden z-30 pointer-events-auto" style={{
      ...
      {rightNodes.map((node, i) => {
        ...
        const startY = CENTER_Y - ((rightNodes.length - 1) * spacing) / 2;
        const y = startY + i * spacing;
        return (<motion.div key={`rn-${i}`} className="absolute flex items-center gap-4 bg-white p-3 pr-6 rounded-xl border border-gray-100 shadow-md z-20 w-[280px] pointer-events-auto" style={{ left: COL_4_X, top: y - 24 }} initial={{ x: 20, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} viewport={{ once: true }} transition={{ delay: 1.2 + (i * 0.1), duration: 0.4 }}>
