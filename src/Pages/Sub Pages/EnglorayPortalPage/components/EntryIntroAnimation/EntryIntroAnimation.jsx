...
const EntryIntroAnimation = ({ onComplete }) => {
    ...
    useEffect(() => {
        ...
        [
            ...
            for (let i = 0; i < count; i++) {
                const u1 = Math.random() * 0.99, u2 = Math.random();
                const gauss = Math.sqrt(-2 * Math.log(u1 + 0.0001)) * Math.cos(2 * Math.PI * u2);
                const gauss2 = Math.sqrt(-2 * Math.log(Math.random() * 0.99 + 0.0001)) * Math.cos(2 * Math.PI * Math.random());
                const gauss3 = Math.sqrt(-2 * Math.log(Math.random() * 0.99 + 0.0001)) * Math.cos(2 * Math.PI * Math.random());
        ...
        gsap.set(['.intro-badge', '.intro-title', '.intro-copy'], { visibility: 'visible', opacity: 0, y: 40 });

        introTl.to(['.intro-badge', '.intro-title', '.intro-copy'], {
            duration: 1.2,
            opacity: 1,
            y: 0,
            stagger: 0.18,
            ...
            <main className="ui-layer">
                <section className="screen intro-screen">
                    <div className="content-center">
                        <span className="intro-badge">ADMISSIONS COHORT 2026</span>
                        <h1 className="intro-title">
                            MASTER REAL<br />
                            <span>ENGINEERING SKILLS</span>
                        </h1>
                        {/* <p className="intro-copy">
                            Engloray bridges academic theory and industry pipelines. Work on production codebases, get direct reviews, and secure verified internships.
                        </p> */}
                    </div>
                    <button
                        type="button"
